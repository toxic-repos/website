...
import { useSearchBar } from 'src/composable/useSearchBar'
import VHeader from 'src/components/VHeader.vue'
import VSearchBar from 'src/components/VSearchBar.vue'
import VDrawer from 'src/components/VDrawer.vue'
import VDrawerButton from 'src/components/VDrawerButton.vue'
import VDrawerSection from 'src/components/VDrawerSection.vue'
import VDrawerButtonIcon from 'src/components/VDrawerButtonIcon.vue'
import type { UseDrawer, UseTheme } from 'types'

export default defineComponent({
  components: { VDrawerButtonIcon, VDrawerSection, VDrawerButton, VDrawer, VHeader, VSearchBar },
  setup() {
    ...
    const drawerItemClick = async (to: string) => {
      await Router.push(to)
      drawer.close()
    }

    return { drawer, themeMode, themeToggle, input, items, onFiltered, drawerItemClick }
  }
})
</script>
...
<template>
  <v-drawer>
    <v-drawer-section>
      ...
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'HomePage'" @click="drawerItemClick('/')">Все</v-drawer-button>
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'IpBlockPage'" @click="drawerItemClick('/ip-block')">Блокировка по IP</v-drawer-button>
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'HostileActionsPage'" @click="drawerItemClick('/hostile-actions')"
        >Враждебные действия</v-drawer-button
      >
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'MalwarePage'" @click="drawerItemClick('/malware')">Вредоносное ПО</v-drawer-button>
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'BrokenAssemblyPage'" @click="drawerItemClick('/broken-assembly')"
        >Сломанная сборка</v-drawer-button
      >
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'PoliticalSlogansPage'" @click="drawerItemClick('/political-slogans')"
        >Политические лозунги</v-drawer-button
      >
      <v-drawer-button :is-active="$router.currentRoute.value.name === 'DdosPage'" @click="drawerItemClick('/ddos')">DDoS</v-drawer-button>
    </v-drawer-section>
    <v-drawer-section>
      <template #title>Действия</template>
      <v-drawer-button href="https://t.me/darksider_bot">Добавить</v-drawer-button>
    </v-drawer-section>
    <template #bottom>
      <div :class="$style.bottomIcons">
        <v-drawer-button-icon href="https://github.com/toxic-repos">
        ...
        </v-drawer-button-icon>
        <v-drawer-button-icon href="https://t.me/toxic_repos">
          <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <v-drawer-button-icon @click="themeToggle">
  ...
  <v-header>
    <v-search-bar v-model="input" :class="$style.searchBar" />
  </v-header>
